...
public class TestIterator {
	...
	public void setUp() throws Exception {
		...
		// DONE does it make any difference?
		// the larger the size gets, ArrayList performs better
		list = new LinkedList<Integer>();
		//when run program, run with arrayList, then LinkedList
	}
	...
	public void testRemove() {
		...
		while (i.hasNext()) {
			if (i.next() == 77) {
				...
				//it tries to remove the value at index 77 instead of the value
				//containing 77
				//list.remove(77);//remove comment and run tests when ready
			}
	...
	public void testAverageValues() {
		...
		// DONE
		final Iterator<Integer> i = list.iterator();
		while (i.hasNext() && n<list.size()){ //while loop added
			sum = sum  + list.get(n);
			n++;
		}
